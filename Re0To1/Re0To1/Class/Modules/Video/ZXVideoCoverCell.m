...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UIImageView *coverView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
        self.coverView = coverView;
        [self.contentView addSubview:coverView];

        UIImageView *playView = [[UIImageView alloc] initWithFrame:CGRectMake((self.bounds.size.width - 50) / 2, (self.bounds.size.height - 50) / 2, 50, 50)];
